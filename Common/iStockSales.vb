            ...
            Using DR As IDataReader = DB.ExecuteReader(DBC)
                ...
                With DR
                    Do While .Read
                        ...
                        Me.SalesNo = Convert.ToInt64(IIf(Not IsDBNull(.Item("SalesNo")), Trim(.Item("SalesNo").ToString), 0))
...
#Region "GetAll Sales Items"
    Public Function GetSalesItemsByDates() As DataSet
        Try

            Dim DB As Database = DatabaseFactory.CreateDatabase(ISTOCK_DBCONNECTION_STRING)
            Dim DBC As DbCommand = DB.GetStoredProcCommand(SALES_GETSTOCKITEMSBYDATES)
            DB.AddInParameter(DBC, "@FromDate", DbType.DateTime, Me.FromDate)
            DB.AddInParameter(DBC, "@ToDate", DbType.DateTime, Me.ToDate)
            Return DB.ExecuteDataSet(DBC)
